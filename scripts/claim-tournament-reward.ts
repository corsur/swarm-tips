/**
 * Claim a player's tournament reward using the merkle proof generated by
 * `finalize-tournament.ts`. Reads
 * `scripts/tournament-{TOURNAMENT_ID}-proofs.json`, finds the entry
 * matching the caller's wallet, and submits claim_reward(amount, proof).
 * Idempotent — the on-chain `player_profile.claimed` flag prevents
 * double-claim.
 *
 * Usage: ANCHOR_WALLET=~/.config/solana/id.json \
 *        ANCHOR_PROVIDER_URL=https://api.mainnet-beta.solana.com \
 *        TOURNAMENT_ID=1 \
 *          npx ts-node scripts/claim-tournament-reward.ts
 */
...
if (!TOURNAMENT_ID) {
  console.error("TOURNAMENT_ID env var is required (e.g. 1, 2).");
  process.exit(1);
}
