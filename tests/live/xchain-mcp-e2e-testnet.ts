...
const signSol = (s: SessionSigner, d: Uint8Array): number[] => Array.from(signDigest(s, d));
const w32 = (hex: string): Uint8Array => fromHex(hex as `0x${string}`, { to: "bytes", size: 32 });
const w20 = (hex: string): Uint8Array => fromHex(hex as `0x${string}`, { to: "bytes", size: 20 });
...
function legComponents() {
  return [
    { name: "chainTag", type: "bytes32" }, { name: "contractId", type: "bytes32" },
    { name: "player", type: "bytes32" }, { name: "sessionKey", type: "address" },
    { name: "stake", type: "uint128" }, { name: "tranche", type: "uint128" },
  ] as const;
}
const SETTLE_ABI = [{
  type: "function", name: "settle", stateMutability: "nonpayable",
  inputs: [
    { name: "cert", type: "tuple", components: [
      { name: "matchId", type: "bytes32" }, { name: "tournamentId", type: "uint64" },
      ...
      { name: "quoteTimestamp", type: "uint64" }, { name: "quoteMaxAgeSecs", type: "uint32" },
      { name: "matchDeadline", type: "uint64" }, { name: "claimWindowSecs", type: "uint32" },
      { name: "aIsP1", type: "uint8" },
    ]},
    { name: "oc", type: "tuple", components: [
      { name: "matchId", type: "bytes32" }, { name: "matchLiveDigest", type: "bytes32" },
      { name: "outcomeKind", type: "uint8" }, { name: "stepCount", type: "uint8" },
      { name: "p1Guess", type: "uint8" }, { name: "p2Guess", type: "uint8" },
      { name: "firstCommitter", type: "uint8" }, { name: "matchupType", type: "uint8" },
      { name: "transcriptHash", type: "bytes32" },
    ]},
    { name: "liveSigs", type: "bytes[3]" }, { name: "ocSigs", type: "bytes[3]" },
  ], outputs: [],
}] as const;
const legAbi = (l: MatchLiveCert["legA"]) => ({
  chainTag: toHex(l.chainTag), contractId: toHex(l.contract), player: toHex(l.player),
  sessionKey: toHex(l.sessionKey), stake: l.stake, tranche: l.tranche,
});
const certAbi = (c: MatchLiveCert) => ({
  matchId: toHex(c.matchId), tournamentId: c.tournamentId, matchupCommitment: toHex(c.matchupCommitment),
  legA: legAbi(c.legA), legB: legAbi(c.legB), quoteTimestamp: c.quoteTimestamp,
  quoteMaxAgeSecs: c.quoteMaxAgeSecs, matchDeadline: c.matchDeadline,
  claimWindowSecs: c.claimWindowSecs, aIsP1: c.aIsP1,
});
const ocAbi = (o: OutcomeCert) => ({
  matchId: toHex(o.matchId), matchLiveDigest: toHex(o.matchLiveDigest), outcomeKind: o.outcomeKind,
  stepCount: o.stepCount, p1Guess: o.p1Guess, p2Guess: o.p2Guess, firstCommitter: o.firstCommitter,
  matchupType: o.matchupType, transcriptHash: toHex(o.transcriptHash),
});
...
function certFromPayload(p: any): MatchLiveCert {
  const leg = (l: any) => ({
    ...
    contract: w32(l.contract), player: w32(l.player), sessionKey: w20(l.session_key),
    stake: BigInt(l.stake_base_units), tranche: BigInt(l.tranche_base_units),
  });
  return {
    matchId: w32(p.match_id), tournamentId: BigInt(p.tournament_id),
    matchupCommitment: w32(p.matchup_commitment ?? `0x${"00".repeat(32)}`),
    legA: leg(p.leg_a), legB: leg(p.leg_b),
    quoteTimestamp: BigInt(p.quote_timestamp), quoteMaxAgeSecs: p.quote_max_age_secs,
    matchDeadline: BigInt(p.match_deadline), claimWindowSecs: p.claim_window_secs,
    aIsP1: p.a_is_p1,
  };
}

(LIVE ? describe : describe.skip)("live MCP cross-chain e2e (deployed game-api)", function () {
  ...
  it("matchmaking -> fund -> relay -> outcome-cosign -> settle both legs", async () => {
    ...
    const player = Keypair.fromSecretKey(
      Uint8Array.from(JSON.parse(readFileSync(`${homedir()}/.config/solana/id.json`, "utf8")) as number[]),
    );
    const evmPlayer = privateKeyToAccount((EVM_KEY.startsWith("0x") ? EVM_KEY : `0x${EVM_KEY}`) as `0x${string}`);
    ...
    await api("/internal/xqueue/join", {
      wallet: evmPlayer.address, chain: EVM_CHAIN, session_key: toHex(legB.address), tournament_id: TOURNAMENT_ID,
    });
    const joined = await api("/internal/xqueue/join", {
      wallet: solWallet, chain: SOLANA_CHAIN, session_key: toHex(legA.address), tournament_id: TOURNAMENT_ID,
    });
    ...
    const solFund = await api("/internal/xqueue/build-sol-fund", { wallet: solWallet });
    ...
    castSend([
      ...
      matchIdHex, toHex(legB.address), toHex(legA.address),
      payload.a_is_p1 === 0 ? "true" : "false",
      String(payload.match_deadline - 300), String(payload.match_deadline),
      "--value", evmStake.toString(),
    ]);
    ...
    await m.lockXtranche(matchIdArr, new BN(payload.leg_a.tranche_base_units))
      .accounts({ xmatch: xmatchPda, pool: xpoolPda, operator: player.publicKey }).rpc();
    castSend(["lockTranche(bytes32,uint128)", matchIdHex, BigInt(payload.leg_b.tranche_base_units).toString()]);
    await poll(() => evmStatus(matchIdHex), (s) => s === 2, "EVM Locked");

    // 5) Relay a co-signed terminal checkpoint (both session keys).
    const cp: Checkpoint = {
      matchLiveDigest: w32(payload.match_live_digest), stepCount: TERMINAL_STEP_COUNT,
      p1Commit: w32(`0x${"11".repeat(32)}`), p2Commit: w32(`0x${"22".repeat(32)}`),
      p1Guess: 0, p2Guess: 0, firstCommitter: 1, matchupType: 0, transcriptHash: w32(`0x${"33".repeat(32)}`),
    };
    const cpDigest = checkpointDigest(cp);
    await api("/internal/xqueue/checkpoint", {
      wallet: solWallet,
      checkpoint: {
        step_count: cp.stepCount, p1_commit: toHex(cp.p1Commit), p2_commit: toHex(cp.p2Commit),
        p1_guess: cp.p1Guess, p2_guess: cp.p2Guess, first_committer: cp.firstCommitter,
        matchup_type: cp.matchupType, transcript_hash: toHex(cp.transcriptHash),
      },
      sigs: [bytesToHex(Uint8Array.from(signDigest(legA, cpDigest))), bytesToHex(Uint8Array.from(signDigest(legB, cpDigest)))],
    });

    // 6) Operator-cosign the outcome (game-api derives it from the checkpoint).
    const ocResp = await api("/internal/xqueue/outcome-cosign", { wallet: solWallet });
    expect(ocResp.outcome_kind).to.equal(HOMOG_BOTH_CORRECT);
    const oc: OutcomeCert = {
      matchId, matchLiveDigest: w32(payload.match_live_digest), outcomeKind: ocResp.outcome_kind,
      stepCount: ocResp.step_count, p1Guess: ocResp.p1_guess, p2Guess: ocResp.p2_guess,
      firstCommitter: ocResp.first_committer, matchupType: ocResp.matchup_type,
    ...
    const evmLive = [signEvm(legA, liveDigest), signEvm(legB, liveDigest), bump27(opLive)];
    const evmOc = [signEvm(legA, ocDigest), signEvm(legB, ocDigest), bump27(opOc)];
    castSend([encodeFunctionData({ abi: SETTLE_ABI, functionName: "settle",
      args: [certAbi(cert) as any, ocAbi(oc) as any, evmLive as any, evmOc as any] })]);
    ...
    const solLive = [signSol(legA, liveDigest), signSol(legB, liveDigest), opLiveSol];
    const solOc = [signSol(legA, ocDigest), signSol(legB, ocDigest), opOcSol];
    const [tournamentPda] = PublicKey.findProgramAddressSync(
      [Buffer.from("tournament"), new BN(TOURNAMENT_ID).toArrayLike(Buffer, "le", 8)], PROGRAM_ID);
    const [playerProfilePda] = PublicKey.findProgramAddressSync(
      [Buffer.from("player"), new BN(TOURNAMENT_ID).toArrayLike(Buffer, "le", 8), player.publicKey.toBuffer()], PROGRAM_ID);
    const [globalConfigPda] = PublicKey.findProgramAddressSync([Buffer.from("global_config")], PROGRAM_ID);
    const gc: any = await (program.account as any).globalConfig.fetch(globalConfigPda);
    const certNoA = {
      matchId: matchIdArr, tournamentId: new BN(payload.tournament_id),
      matchupCommitment: Array.from(cert.matchupCommitment),
      legB: {
        chainTag: Array.from(cert.legB.chainTag), contract: Array.from(cert.legB.contract),
        player: Array.from(cert.legB.player), sessionKey: Array.from(cert.legB.sessionKey),
        stake: new BN(cert.legB.stake.toString()), tranche: new BN(cert.legB.tranche.toString()),
      },
      quoteTimestamp: new BN(cert.quoteTimestamp.toString()), quoteMaxAgeSecs: cert.quoteMaxAgeSecs,
      matchDeadline: new BN(cert.matchDeadline.toString()), claimWindowSecs: cert.claimWindowSecs, aIsP1: cert.aIsP1,
    };
    const outcomeArg = {
      matchId: matchIdArr, matchLiveDigest: Array.from(liveDigest), outcomeKind: oc.outcomeKind,
      stepCount: oc.stepCount, p1Guess: oc.p1Guess, p2Guess: oc.p2Guess, firstCommitter: oc.firstCommitter,
      matchupType: oc.matchupType, transcriptHash: Array.from(oc.transcriptHash),
    };
    await m.settleXmatch(certNoA, outcomeArg, solLive, solOc).accounts({
      xmatch: xmatchPda, pool: xpoolPda, tournament: tournamentPda, playerProfile: playerProfilePda,
      globalConfig: globalConfigPda, treasury: gc.treasury, player: player.publicKey,
      cranker: player.publicKey, systemProgram: SystemProgram.programId,
    }).rpc();

    expect(await evmStatus(matchIdHex)).to.equal(4);
    const xmatch: any = await (program.account as any).xChainMatch.fetch(xmatchPda);
...
async function submitMatchmakerCosigned(
  connection: Connection, player: Keypair, solFund: any,
): Promise<void> {
  ...
  const mmSlot = tx.signatures.find((s) => !s.publicKey.equals(player.publicKey));
  if (!mmSlot) throw new Error("build-sol-fund tx has no matchmaker signer slot");
  tx.addSignature(mmSlot.publicKey, Buffer.from(solFund.matchmaker_signature, "base64"));
