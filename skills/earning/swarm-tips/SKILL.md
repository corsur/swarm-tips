...
- **Some client-side tools require you to be the campaign owner** (`shillbot_approve_task`, `shillbot_list_pending_approval`) — the on-chain instruction enforces the wallet match. Decline by withholding approval; permissionless expiry later returns escrow.
