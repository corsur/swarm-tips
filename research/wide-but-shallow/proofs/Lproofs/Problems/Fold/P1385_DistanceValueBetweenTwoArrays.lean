...
def distValue (arr1 arr2 : ℕ → ℕ) (m1 m2 d : ℕ) : ℕ :=
  (List.range m1).countP fun i => farFromAll arr2 m2 (arr1 i) d

/-- SCHEME (fold): the distance value is a streaming right-fold tally over the indices of `arr1`. -/
theorem cls (arr1 arr2 : ℕ → ℕ) (m2 d : ℕ) :
    IsRightFold (fun L : List ℕ => L.countP fun i => farFromAll arr2 m2 (arr1 i) d) := by
...
theorem corr (arr1 arr2 : ℕ → ℕ) (m1 m2 : ℕ) : Antitone (distValue arr1 arr2 m1 m2) := by
  intro a b hab
