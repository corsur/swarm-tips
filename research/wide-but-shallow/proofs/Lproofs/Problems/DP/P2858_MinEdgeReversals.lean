...
def reroot (e : ℕ × ℕ) (a b : ℕ) (cAt : ℕ) : ℕ :=
...
theorem cls (e : ℕ × ℕ) (a b cAt : ℕ) :
    reroot e a b cAt = if e = (a, b) then cAt + 1 else cAt - 1 := rfl
...
theorem corr (e : ℕ × ℕ) (a b cAt : ℕ) :
    (oriented e a b → reroot e a b cAt = cAt + 1) ∧
    (¬ oriented e a b → reroot e a b cAt = cAt - 1) := by
  refine ⟨fun h => ?_, fun h => ?_⟩
  · simp only [reroot, oriented] at *; rw [if_pos h]
  · simp only [reroot, oriented] at *; rw [if_neg h]
