...
def lps (s : ℕ → Char) : ℕ → ℕ → ℕ → ℕ
  | 0, _, _ => 0
  | f + 1, i, j =>
      if j ≤ i then (if i = j then 1 else 0)
      else if s i = s j then 2 + lps s f (i + 1) (j - 1)
      else max (lps s f (i + 1) j) (lps s f i (j - 1))
