...
def best (coin : ℕ → ℕ → ℤ) (len : ℕ) : ℕ → ℕ → ℕ → ℕ → ℤ
  | 0, _, _, _ => 0
  | f + 1, pos, lane, sw =>
    if pos ≥ len then 0 else coin pos lane + nextMove coin (best coin len f) pos lane sw

/-- SCHEME (dp): an in-range cell decomposes into its coins plus the best onward move --- the
    stay/switch recurrence. -/
theorem cls (coin : ℕ → ℕ → ℤ) (len f pos lane sw : ℕ) (h : pos < len) :
    best coin len (f + 1) pos lane sw =
      coin pos lane + nextMove coin (best coin len f) pos lane sw := by
...
theorem corr (coin : ℕ → ℕ → ℤ) (len f pos lane : ℕ) :
    Monotone (best coin len f pos lane) := by
