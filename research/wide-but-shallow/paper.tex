...
each scheme; ``certified'' is how many additionally carry a Lean proof that the idea solves \emph{that}
problem, not just some problem in the scheme (Section~\ref{sec:lean}). 76 of 100 are certified (95\% Wilson
interval $[0.67,0.83]$). The ``Lean witness'' is the formal test each scheme is checked against (a
\emph{catamorphism} is a fold over a tree; a \emph{least fixpoint} is where graph relaxation settles). Every
assigned problem carries a proof that compiles (78 of 78), and 76 of those prove a property of the specific
problem. The two that fall short---Burst Balloons and Block Placement Queries---would need a longer,
multi-part proof than one self-contained file allows, so we leave them uncounted. Among the 76 the strength
still varies: full correctness for the flagship folds of Section~\ref{sec:lean}, and a one-directional
property (such as ``everything it reports is valid'') otherwise.}
\label{tab:schemes}
\end{table}
...
100 problems we prove two things. First, \textbf{the idea fits} (\texttt{cls}): the standard solution really
does have the shape of its assigned idea. Second, \textbf{the solution is sound} (\texttt{corr}): that
solution is proven to satisfy a correctness property of the problem (often one direction of the guarantee,
e.g.\ ``everything it reports is valid''). A problem counts only when both proofs pass with no gaps (Lean
flags any unfinished proof; there are none), use only Lean's standard logical axioms, \emph{and} the
correctness proof is about the \textbf{specific problem}---its actual grid, array, or condition---rather than
a generic statement that every problem in the same family would satisfy equally. That last requirement is
the one that matters: a generic proof certifies the \emph{category}, not the \emph{problem}, so we exclude
such proofs automatically (the check ships in the build script). Two problems clear only the generic
bar---Burst Balloons and Block Placement Queries, whose real guarantees need more machinery than a single
self-contained proof allows---so we leave them out, giving 76.
...
and a cautious one.} 76\% is a sample estimate, so the confidence interval $[67\%,83\%]$ is the honest
summary, not the point value. It errs low in two ways: a problem we cannot formalize stays uncounted, and
the fixed rule sends borderline cases---some ``greedy'' and ``simulation'' problems that are really single
passes or binary searches---to the tail rather than into a scheme. So 76\% is, if anything, a lower bound on
how many problems reduce to the four ideas; but that is a property of our method, not a promise about
problems we did not sample. \textbf{Window dependence.} The company difference is not
