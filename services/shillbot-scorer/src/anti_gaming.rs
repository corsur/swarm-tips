...
fn detect_view_velocity_anomaly(total_views: u64, snapshots: &[ViewSnapshot]) -> bool {
    ...
    for i in 0..max_pairs {
        ...
        // total_views > 0 is guaranteed by the guard at function entry.
        let percent = view_diff
            .saturating_mul(100)
            .checked_div(total_views)
            .expect("total_views is nonzero (guarded at function entry)");
...
fn detect_engagement_ratio_anomaly(metrics: &EngagementMetrics) -> bool {
    ...
    // metrics.views > 0 is guaranteed by the guard above.
    let like_percent = metrics
        .likes
        .saturating_mul(100)
        .checked_div(metrics.views)
        .expect("metrics.views is nonzero (guarded above)");
