...
- `xchain_build_lock` — [STATE] build the unsigned EVM permissionless `lockTranche` tx so the EVM-leg player locks their cross-chain payout tranche after both legs fund (settle requires Locked). Reconstructs the cert from the relay payload (chain tags recomputed as `keccak256(chain id)` to match game-api) and uses the operator's pairing match-live signature (`operator_signature`) as authorization — no operator action. Returns `{to, data, value_wei: 0, chain, match_id}`; caller pays only gas. `build_evm_lock_call` in `src/xchain.rs` via `evm_chain::build_lock_tranche_parts`. (Solana-leg lock is a follow-up needing a game-api `build-sol-lock` endpoint.)
