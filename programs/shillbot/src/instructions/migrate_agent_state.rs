...
//! PDAs from the original 41-byte layout to the current 90-byte layout.
//!
//! Background. Sprint 0 of the v5 roadmap-iteration discovered (2026-05-06)
//! that `AgentState` PDAs created against very early versions of the program
//! (when the struct was just `{ agent: Pubkey, bump: u8 }`) sit on disk at
//! `8 disc + 32 + 1 = 41` bytes. The current struct expects 90 bytes, so
//! Anchor's typed deserializer fails on these legacy accounts with
//! `AccountDidNotDeserialize` (3003) — observed for the founder wallet
//! `CKsZ7ZMLLUzbHUeu2Vm5mjuB8QQi3vfvqvXFdFxT7xmY` on devnet, PDA
//! `67NVWtF4j1niHXR4zBRRBytru8TFC3959u6SfhCE5btn`. The earlier #12 reputation
//! extension (2026-05-02) was bytewise-compatible because it kept SPACE at 90
//! and only re-carved a `_reserved[32]` block; the much older 41-byte case is
//! the real migration gap.
//!
//! Design.
//!   * The instruction takes the legacy account as a raw `AccountInfo` —
//!     `Account<AgentState>` would fail to open the underlying 41-byte data
...
//!   * The legacy bump is at offset 40; the new layout puts bump at offset 89.
//!     We extract the old bump first, then realloc, then write the entire
//!     new layout (zero-initialized counters, original agent + bump).
...
//! ```text
//! OLD (41 bytes total):                     NEW (90 bytes total):
//!   [0..8]   discriminator                    [0..8]   discriminator
//!   [8..40]  agent (32)                       [8..40]  agent (32)
//!   [40]     bump (1)                         [40]     claimed_count = 0
//!   --                                        [41..49] total_completed = 0
...
const LEGACY_AGENT_STATE_SIZE: usize = 8 + 32 + 1; // 41
const CURRENT_AGENT_STATE_SIZE: usize = AgentState::SPACE; // 90

/// One-shot migration of a 41-byte legacy `AgentState` PDA to the current
...
pub fn migrate_agent_state(ctx: Context<MigrateAgentState>) -> Result<()> {
    ...
    // The migration only handles the legacy 41-byte case. Anything else
    ...
    // Read the old layout: discriminator [0..8], agent [8..40], bump [40].
    let (old_agent, old_bump) = {
        let data = agent_state_info.try_borrow_data()?;
        require!(
    ...
    // Effects: write the new layout. All counters start at 0 — the legacy
    // account had no counters, so there's nothing to migrate forward.
...
mod tests {
    ...
    #[test]
    fn legacy_size_constant_matches_old_layout() {
        // 8 disc + 32 agent + 1 bump
        assert_eq!(LEGACY_AGENT_STATE_SIZE, 41);
    }
    ...
    /// Builds a legacy 41-byte buffer and walks the same byte-extraction the
    /// handler does. Asserts the stored bump and agent pubkey come back out
    /// at the right offsets. This is the regression guard for "someone
    /// reordered the legacy layout assumptions" — pure unit test, no
    /// runtime context.
    #[test]
    fn legacy_bytes_extract_correctly() {
    ...
    /// the handler but runs in unit-test context.
    #[test]
    fn post_migration_layout_round_trips() {
        let agent_pubkey = Pubkey::new_unique();
        let bump: u8 = 200;
        ...
        assert_eq!(parsed.claimed_count, 0);
